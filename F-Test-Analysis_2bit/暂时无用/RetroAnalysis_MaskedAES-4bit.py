...
class RetroAnalysis_MaskedAES(object):  # 定义RetroAnalysis_MaskedAES类
        ...
        def OnePointCollapsedFtest(self,Traces,k,Kp,fo=None):
                """
                论文中的F检验方法
                :param Traces: 输入轨迹
                :param k: 密钥矩阵（N*16）
                ...
                for i in range(8):
                        ...
                        for j in range(0, 8):
                                if (BitChoice[j] == False): # 当此位置为False时，记录该位置子密钥
                                        ...
                                        m = m + 1
                        pv = maf.Full1vsFull2(Traces, Xm, m, Kp, 8) # 判断当前位置是否有影响
                        if (pv < threshold): # 判断没有了i位置的X矩阵与完整输入是否有类似的表现
                                print('Delete bit {0}, pv={1}'.format(i, pv))  # 表示当前位置不涉及泄露
                        else:
                                print('Cannot delete bit {0}, pv={1}'.format(i, pv)) # 表示当前位置有泄露
                                BitChoice[i] = False
        ...
        def CollapsedFtest_Key_data(self, Traces, Key, filename="Ftest_key.txt"):  # 使用外部的数据作为输入
                ...
                k0=np.packbits(k[:,0:4], axis=1)  #每个数据子密钥前8位打包（即将8位合并变为10进制），这是第一份子密钥k0（长为数据量，每个代表当前这份数据前8位的值）
                k1=np.packbits(k[:,4:8], axis=1) # 第二份子密钥k1与上面类似
                Kp=(k0.astype('uint16')<<16)^k1  # 每个数据密钥的十进制值
                N_all, point = Traces.shape # 取出轨迹矩阵的数量和特征点数
                for i in range(point): # 分别循环每个特征点实验
                        print('Sample {0}'.format(i))
                        fo.write('Sample {0}\r\n'.format(i))
                        self.OnePointCollapsedFtest(Traces[:,i], k, Kp[:,0])
                        # 这里的输入分别是，第i特征点的轨迹，密钥矩阵（N*16），转成10进制的密钥（N），和储存文件名
                fo.close()
...
if __name__ == '__main__':
                ...
                N = 100000 # 选取的数据量（注意：数据量一定要多余密钥的所有组合数，不然没有检验结果）
                point = 2000
                filename = "Ftest_key.txt"
                with h5py.File(file, 'r') as f:
                        Key = f['key'][0:N,0:8]
                        Key = Key.astype(np.uint8)
                        plaintext = f['plaintext'][0:N,:]
                        Traces = f['traces'][0:N,:]
                RetroAnalysis_MaskedAES().CollapsedFtest_Key_data(Traces, Key, filename)  # 直接做F检验
