...
class RetroAnalysis_MaskedAES(object):  # 定义RetroAnalysis_MaskedAES类
        ...
        def CollapsedFtest_Key_data(self, Traces, Key, bit_start, filename="Ftest_key.txt"):  # 使用外部的数据作为输入
                ...
                k0=np.packbits(k[:,0:16], axis=1)  #每个数据子密钥前8位打包（即将8位合并变为10进制），这是第一份子密钥k0（长为数据量，每个代表当前这份数据前8位的值）
                k1=np.packbits(k[:,16:32], axis=1) # 第二份子密钥k1与上面类似
                Kp=(k0.astype('uint16')<<16)^k1  # 每个数据密钥的十进制值
...
if __name__ == '__main__':  # 这一版本是4bit使用（32个位置）
                threshold = 10  # 检验的阈值
                # file = 'E:\\PZH\\code\\F-Test-Analysis\\data\\SR32bit01_66W_9267_2024_12_12_21h21m56s_0.98574_byte32.mat'
                file = 'E:\\PZH\\code\\F-Test-Analysis\\data\\SR32bit01_30W_6802_2025_2_11_17h8m19s_0.99678_byte32.mat'
                ...
                point = 6802
                bit_start = 0 # 密钥检测结束的位置~
                # bit_end = bit_start+32  # 结束的位置（这里是4bit用）
                bit_end = bit_start+32  # 结束的位置（这里是2bit用）
