...
class RetroAnalysis_MaskedAES(object):  # 定义RetroAnalysis_MaskedAES类
        ...
        def OnePointCollapsedFtest(self,Traces,k,Kp, bit_start, fo=None):
                """
                论文中的F检验方法
                :param Traces: 输入轨迹
                :param k: 密钥矩阵（N*16）
                ...
                # 2、找到泄露程度d（这部分用的F检验公式是论文公式提到的，算是标准的F检验公式）
                ...
                # 12.13改造，只检测2和1泄露程度
                [Ke2, count2] = maf.BuildRegressionBasis(Kp, 16, 2) # # Ke2：[30W，137]，137是 16个子密钥 + 120子密钥组合 +常数项。每个项值是密钥的组合值
                        # pv=maf.LRA1vsLRA2(Traces,Ke2, count2, Ke4, count4)
                        # print('4-2, Fpv={0}'.format(pv))
                        # if (pv > threshold): # 如果p值大于阈值，说明有明显差异，则泄露程度等于4
                        #         degree = 4
                        # else: # 如果p值小于阈值，泄露度为4的模型与全模型表现相当
                [Ke1, count1] = maf.BuildRegressionBasis(Kp, 16, 1)  # Ke1：[30W，17]，17是 16个子密钥 + 常数项
                ...
                for i in range(0, 16):
                        if (BitChoice[i] == False): # 等于这个值的位置都是有泄露的
                                ...
                                print('bit {0}\t'.format(i+bit_start))
                                if (fo is not None):
                                        fo.write('bit {0}\t'.format(i+bit_start))
                                Rb.append(i)
                                l = l + 1  # 有泄露的位置的数量
                print('\n')
                if (fo is not None):
                ...
                iList = [] # 储存符合条件的组合（位置，比如192转成二进制就是最高两位为1，即11000000）
                hwList = [] # 储存符合HW的组合（标记有几个子密钥组成）
                ...
                # 5.测试剩余可能泄露项的的相关性
                find=False
                for i in iList: # 遍历符合条件的位置组合
                        Xm = (k[:, 0] & 0x0).astype('uint16') # 初始化全0矩阵，用于储存所有数据中当前密钥组的值，比如密钥值为102，密钥组选1和2位置，则值为3（取二进制后密钥相关值，类似于拼凑）
                        m = 0 # 初始化最终有泄露的组合数
                        ...
                        if (pv > threshold and find==False):  # 证明该项与泄露有关
                                ...
                                for j in range(l):
                                        ...
                                        if ((i >> j) & 0x1 == 1):
                                                ...
                                                print('bit {0}\t'.format(Rb[j]+bit_start))
                                                if (fo is not None):
                                                        fo.write('bit {0}\t'.format(Rb[j]+bit_start))
                                print('\n')
                                if (fo is not None):
                                        fo.write('\r\n')
                fo.flush()
        ...
        def CollapsedFtest_Key_data(self, Traces, Key, bit_start, filename="Ftest_key.txt"):  # 使用外部的数据作为输入
                fo = open(filename, "w")
                print('\n\n\nPart 3: Collasped F-test for key')
                ...
                for i in range(point): # 分别循环每个特征点实验
                        print('Sample {0}'.format(i))
                        fo.write('Sample {0}\r\n'.format(i))
                        self.OnePointCollapsedFtest(Traces[:,i], k, Kp[:,0], bit_start, fo)
                        # 这里的输入分别是，第i特征点的轨迹，密钥矩阵（N*16），转成10进制的密钥（N），和储存文件名
                fo.close()
...
if __name__ == '__main__':  # 这一版本是8bit使用（16个位置）
                threshold = 0.1 # 检验的阈值
                file = 'E:\\PZH\\code\\F-Test-Analysis_2bit\\data\\SR2bit01_30W_6802_2025_2_20_11h6m1s_0.99623_byte64_noise10_FSLA4_1000.mat'
                # file = 'E:\\PZH\\code\\F-Test-Analysis_2bit\\data\\SR2bit01_30W_6802_2025_2_20_11h6m1s_0.99623_byte64.mat'
                ...
                N = 300000 # 选取的数据量（注意：数据量一定要多余密钥的所有组合数，不然没有检验结果）
                bit_start = 0 # 密钥检测开始的位置
                bit_end = bit_start+16  # 结束的位置（这里是4bit用）
                # bit_end = bit_start+64  # 结束的位置（这里是2bit用）
                # filename = "Ftest_key_6802点_4bitpv10.txt"
                # filename = "Ftest_key_490点_2bitpv0.1_noise10_1.txt"
                filename = "Ftest_key_FSLA1000点_2bitpv0.1_noise10_1.txt"
                with h5py.File(file, 'r') as f:
                        Key = f['key'][0:N,bit_start:bit_end]
                        Key = Key.astype(np.uint8)
                        # plaintext = f['plaintext'][0:N,:]
                        Traces = f['traces'][0:N,:]
                RetroAnalysis_MaskedAES().CollapsedFtest_Key_data(Traces, Key, bit_start, filename)  # 直接做F检验
                end_time = timeit.default_timer() # 结束时间
                time = (end_time-start_time)/60.0
                print("program takes {0} minutes process time".format(time)) # 2bit检测16给=个位置时间大概10小时
                fo = open(filename, "a")
                fo.write(f"评测时间为:{time}\n")
