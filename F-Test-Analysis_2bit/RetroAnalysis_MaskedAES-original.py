...
class RetroAnalysis_MaskedAES(object):  # 定义RetroAnalysis_MaskedAES类
        ...
        def OnePointCollapsedFtest(self,Traces,k,Kp,fo=None):
                ...
                [Ke4, count4] = maf.BuildRegressionBasis(Kp, 16, 4) # 构建回归基（交互项），输入分别为合并的密钥，子密钥个数和泄露程度
                # Ke4为X的交互矩阵，count是交互项数

                pv=maf.LRAvsFull(Traces,Kp, 16, Ke4, count4) # 计算当前泄露度d的交互模型的p值
                print('16-4, Fpv={0}'.format(pv))
                if(pv>threshold): # 如果p值大于阈值，则泄露程度等于16
                        degree=16 # 最初的子密钥
                else: # 如果p值小于阈值，继续验证
                        ...
                        if (pv > threshold): # 不符合则泄露程度为4
                                degree = 4
                        else: # 如果p值小于阈值，继续验证更小的泄露程度
                ...
                for i in range(16):
                        ...
                        for j in range(0, 16):
                                if (BitChoice[j] == False): # 当此位置为False时，删除该位置
                ...
                for i in iList: # 遍历符合条件的位组合
                        ...
                        m = 0 # 初始化最终有泄露的组合数
                        for j in range(l): # 遍历泄露的位置数l
                                if ((i >> j) & 0x1 == 1): # 如果第j给位置位1
                        ...
                        if (pv > threshold and find==False):  # 证明该项与泄露有关
                                # find=True # 将此项标记为已经搜查（不要了，影响观看所有结果）
                                if (fo is not None):
                                        # 输出当前的组合i和p值
        ...
        def CollapsedFtest_Key_data(self, data, N,filename="Ftest_key.txt"):  # 使用外部的数据作为输入
                fo = open(filename, "w")
                print('\n\n\nPart 3: Collasped F-test for key')
                if N == -1:
                        Traces = data['traces']
                        ...
                        Key = data['key'][0:N, :]
                        Plain = data['plaintext'][0:N, :]
                k=np.copy(Key) # 复制密钥副本
                k0=np.packbits(k[:,0:8], axis=1)  #每个数据子密钥前8位打包（即将8位合并变为10进制），这是第一份子密钥k0（长为数据量，每个代表当前这份数据前8位的值）
                k1=np.packbits(k[:,8:16], axis=1) # 第二份子密钥k1与上面类似
                Kp=(k0.astype('uint16')<<8)^k1  # 每个数据密钥的十进制值a
...
if __name__ == '__main__':
                threshold = 3  # 检验的阈值
                file = 'E:\\PZH\\code\\F-Test-Analysis\\data\\Retro_MaskedAscon_NoBI_20kT_8000Samples_CollapsedKey8.npz'
                ...
                data = np.load(file)
                RetroAnalysis_MaskedAES().CollapsedFtest_Key_data(data)  # 直接做F检验
